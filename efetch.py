...
class EFetch(ELink, EPost, ESearch):
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(f"EFetch did not complete successfully (HTTP {response.status_code})")
    ...
    def results(self):
        ...
        try:
            self._webenv, self._querykey, self._fetchdata = self.get()
        except ValueError:
    ...
    def __repr__(self):
        ...
        if self._status.value >= state.ESEARCH.value:         
             self._repr += f"ESearch<'{self._term}', {self._dbfrom}'>"

        if self._status.value >= state.ELINK.value:             
             self._repr += f":ELink<'{self._dbfrom}', '{self._db}'>"
          
        if self._status.value >= state.EPOST.value:
             self._repr += f":EPost<'{self._db}', '{str(self._ids)}'>"

        self._repr += f":EFetch<'{self._db}', '{self._ids}'>"
...
def esearch_elink_efetch(query, dbfrom="pubmed", dbto="protein", 
    ...
    elink   = ELink(dbto, dbfrom=dbfrom, retmode=retmode,
                   source=esearch)
