    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(f"[OBJECTS:EFETCH] EFetch did not complete successfully (HTTP {response.status_code})")
                logging.error(f"[OBJECTS:EFETCH] Error Message : {response.text}")
                return ""
